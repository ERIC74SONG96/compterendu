...
export default function CompteRenduApp() {
  ...
  const charger = useCallback(async () => {
    ...
    try {
      const res = await window.storage.list("rapport:", true);
      ...
      const valides = charges.filter(Boolean);
      valides.sort((a, b) => (b.ts || 0) - (a.ts || 0));
      setRapports(valides);
    } catch (e) {
      // Aucun rapport encore, ou stockage indisponible
      try {
        // list peut échouer si préfixe vide selon l'environnement : on distingue
        setRapports([]);
      } catch { /* noop */ }
    }
    setChargement(false);
  ...
  const enregistrer = async () => {
    ...
    try {
      const ok = await window.storage.set(`rapport:${id}`, JSON.stringify(donnees), true);
  ...
  const supprimer = async (id) => {
    try {
      await window.storage.delete(`rapport:${id}`, true);
